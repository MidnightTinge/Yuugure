      ...
      if (validateMethods(exchange, Methods.GET, Methods.POST)) {
        if (exchange.getRequestMethod().equals(Methods.GET)) {
          ...
          if (form != null) {
            ...
            var frmRating = form.getFirst("rating");
            if (frmRating == null || frmRating.getValue().isBlank()) {
              uploadResult.addError("Rating is required.");
            } else {
              var rating = frmRating.getValue().toLowerCase().trim();
              if (!(rating.equals("safe") || rating.equals("questionable") || rating.equals("explicit"))) {
                uploadResult.addError("Invalid rating. Expected one of \"safe\"|\"questionable\"|\"explicit\"");
              } else {
                // Ensure we have tags
                            ...
                            if (mtx.acquire()) {
                              dbtags = App.database().jdbi().withHandle(h -> {
                                ...
                                try {
                                  ...
                                  var ratingTag = App.tagManager().ensureTag(new TagDescriptor(TagCategory.RATING, rating));

                                  ret.addAll(ensured.tags);
                                  ret.addAll(ratingTag.tags);
